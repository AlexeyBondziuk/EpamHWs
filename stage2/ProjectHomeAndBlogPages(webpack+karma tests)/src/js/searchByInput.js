...
const inputForGenre = document.querySelector('#inputForAuthor'),
    inputForTitle = document.querySelector('#inputForTitle'),
    dropdownForGenre = document.getElementById('dropdownForAuthors'),
    dropdownForTitles = document.getElementById('dropdownForTitles'),
    inputWrapperGenre = document.querySelector('.blog-section_input-wrapper-author'),
...
inputForGenre.value = getItemFromLocalStorage('author')
...
dropdownForTitles.addEventListener('click', (e) => {
    setItemToLocalStorage(e, 'title', 'author', inputForGenre, inputForTitle, dropdownForTitles)
    inputForGenre.disabled = true
})

inputForGenre.addEventListener('click', () => {
    showHideDropdown(dropdownForGenre)
    hideDropdown(dropdownForTitles)
})
inputForGenre.addEventListener('keyup', () => {
    filterFunction(inputForGenre, 'noAuthor', dropdownForGenre, 'No such genre')
})
inputForGenre.addEventListener('change', () => {
    removeItemFromLocalStorage(inputForGenre, 'author', dropdownForGenre)
})

inputWrapperGenre.addEventListener('focusout', (e) => {
    const didClickMenu = dropdownForGenre.contains(e.relatedTarget);
    if (!didClickMenu) {
        hideDropdown(dropdownForGenre)
    }
})

dropdownForGenre.addEventListener('click', (e) => {
    setItemToLocalStorage(e, 'author', 'title', inputForTitle, inputForGenre, dropdownForGenre)
})
...
function setInfoForSearchByGenre(searchData, numberOfFilm, genre, backdrop, postType, filmTitle) {
...
function setAuthorsToDropdown(searchData) {
    let setForGenre = new Set();

    for (let prop in searchData) {
        if (searchData.hasOwnProperty(prop)) {
            searchData[prop].genre.forEach(item => {
                setForGenre.add(item)
            })
        }
    }

    const isDropDownForGenre = true

    innerToDropdown(null, dropdownForGenre, searchData, isDropDownForGenre, setForGenre)
}
...
export { createSearchSelectedPosts, deepClone, removeSearchSelectedPosts, setInfoForSearchByGenre,
    setAuthorsToDropdown, searchSelectedPostWrapper, getItemFromMovieCollection, inputForGenre}