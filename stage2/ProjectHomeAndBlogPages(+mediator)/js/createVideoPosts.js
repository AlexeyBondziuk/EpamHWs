...
function createVideoPosts(rootDiv, numberOfVideoPosts, mainDataForPost, searchable, searchData) {
    ...
    for (let numberOfFilm = 0; numberOfFilm < numberOfVideoPosts; numberOfFilm++) {
        ...
        getInfo(numberOfFilm).then(responseVideoRequest => {
            ...
            setInfoForSearchByAuthor(searchData, numberOfFilm, genre, backdrop, 'video', filmId)
            ...
            readMoreButton.addEventListener('click',() => videoPostData.goToFilmPage())
            ...
            iconPlay.addEventListener('click', () => {
                videoPostData.clickToPlayIcon()
            })

            const popUpMessageData = new PopUpMessageData('info', 'Are you sure you want to delete this post?',
                2, 'dark', true)

            $(`#deletePostButton_${videoPostData.filmId}`).showPopup(popUpMessageData)
           ...
           <div class="blog-section_blog-post_video">
                 <div id="rootForChangeVideoPost${videoPostData.filmId}">
                    <div class="img-wrapper">
                        ...
                        <img id="iconOfPlay${videoPostData.filmId}" class="icon-play" src="./img/a-icon-play.svg" 
                        alt="icon of play">            
                    </div>
                 </div>
                 <div class="blog-post-main-content">
                    ...
                    <div class="blog-post_corner-icon">
                        <img class="blog-post-main-content_content-icon" src="./img/a-icon-playmini.svg" 
                             alt="image of play">
                    </div>      
                 </div>        
           </div>    
