...
function createTextPosts(rootDiv, numberOfTextPosts, numberOfPhotoPosts, numberOfAudioPosts, numberOfVideoPosts,
    ...
    for (let i = 0; i < numberOfTextPosts; i++) {
        ...
        setInfoForSearchByAuthor(searchData, numberOfFilm, genre, backdrop, 'text', filmId)
        ...
        readMoreButton.addEventListener('click',() => textPostData.goToFilmPage())

        const popUpMessageData = new PopUpMessageData('info', 'Are you sure you want to delete this post?',
            2, 'dark', true)

        $(`#deletePostButton_${textPostData.filmId}`).showPopup(popUpMessageData)
    }
            ...
            <div class="blog-section_blog-post_text">
                <div class="blog-post-main-content">
                    ...
                    <div class="blog-post_corner-icon">
                        <img class="blog-post-main-content_content-icon" src="./img/a-icon-text.svg" 
                                alt="image of melody">
                    </div>
                </div>
            </div>
